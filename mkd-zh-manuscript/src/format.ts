...
export type ZhManuscriptTocEntry = {
  kind: 'chapter';
  index: number;
  title: string;
...
export type ZhManuscriptTocAst = {
  type: 'zh-manuscript-toc';
  title?: string;
  entries: ZhManuscriptTocEntry[];
};

export type ZhManuscriptExpandingProbeGrowthEvent = {
...
export type ZhManuscriptExpandingProbeAst = {
  type: 'zh-manuscript-expanding-probe';
  enabled: boolean;
  label: string;
  region: {
    id: string;
  ...
  growthHistory: ZhManuscriptExpandingProbeGrowthEvent[];
};
 ...
 */
function formatCharCount(raw: string): string {
...
export class ZhManuscriptFormat {
  ...
  private readonly tocEntries: ZhManuscriptTocEntry[] = [];
  ...
  private readonly expandingProbeGrowthHistory: ZhManuscriptExpandingProbeGrowthEvent[] = [];
  ...
  constructor(private readonly config: Record<string, unknown>) {
    ...
    this.footnoteHeading = asString(asRecord(this.manuscriptConfig.footnotes).heading, '注释');
  ...
  flush(ctx: FormatContext): void {
    ...
    for (let i = 0; i < buffer.length; i++) {
      ...
      if (node.kind === 'p') {
        this.growExpandingProbeForNode(node);
        // Chinese typesetting: all paragraphs use the same indent (首行缩进), including
        // the first paragraph after a chapter heading. We still track nextParagraphIsFirst
        // so the caller can override via config if they prefer Western-style no-indent-first.
    ...
    if (ctx.registeredFootnoteCount() > 0) {
      ...
      for (const entry of ctx.registeredFootnotes()) {
        const fallback = [{ kind: 'text', value: `[缺少注释: ${entry.identifier}]` } as SemanticNode];
  ...
  private handleTitleAndCover(titleNode: SemanticNode, nextNode: SemanticNode | undefined, ctx: FormatContext): boolean {
    ...
    // Accept both 'char-count' (English key) and '字数' (Chinese key) for flexibility.
    const rawCharCount = fields['char-count'] || fields['字数'] || fields['word-count'] || '';

    // ── Title page layout ────────────────────────────────────────────────────────
    // 1. Large centered title (the visual anchor of the cover page)
    this.emitWithProperties(ctx, 'cover-title', titleNode.children || [], titleNode, undefined, {
      _coverFields: { author: authorValue, 'char-count': rawCharCount },
      pageOverrides: { header: null, footer: null }
    });

    // 2. Author name centered below the title (display byline, no "By" prefix)
    const byline = fields['byline'] || authorValue;
    if (byline) {
      this.emitWithProperties(ctx, 'cover-line', byline, titleNode, { fontSize: 13 }, {
    ...
    // 3. Submission details — contact and character count — in small type below author.
    //    Combined onto as few lines as possible to keep the page uncluttered.
    ...
    if (rawCharCount) {
      this.emitWithProperties(ctx, 'cover-line', formatCharCount(rawCharCount), titleNode, undefined, {
        _coverKey: 'char-count',
        _coverValue: rawCharCount,
        pageOverrides: { header: null, footer: null }
      });
    }

    // 4. Agent / rights at the bottom (separate-cover-page only gets the push-down margin)
    const footerKeys = ['agent', 'rights'] as const;
    const footerCount = footerKeys.filter((key) => !!fields[key]).length;
    const footerMarginTop = footerCount > 1 ? 120 : 160;
  ...
  private emitEpigraphFromRaw(raw: string, node: SemanticNode, ctx: FormatContext): void {
    ...
    if (lines.length > 0) {
      ...
      // Accept both Western em-dash and Chinese dash (——) as attribution markers.
      const attrMatch = /^[-\u2014\u2013\uff0d]{1,2}\s*(.+)$/.exec(last);
      if (attrMatch) {
  ...
  private resolveSceneBreakText(node: SemanticNode): string {
    ...
    if (/^[#*\-_.\s※◇〇]+$/.test(headingText)) return this.sceneBreakSymbol;
    return headingText;
  }

  getTocAst(): ZhManuscriptTocAst {
    return {
      type: 'zh-manuscript-toc',
  ...
  getExpandingProbeAst(): ZhManuscriptExpandingProbeAst {
    return {
      type: 'zh-manuscript-expanding-probe',
